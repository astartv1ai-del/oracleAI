...
Only synthetic facts are used. The runner exercises the real SQLite repository
...
async def run() -> dict:
    with tempfile.TemporaryDirectory(prefix="oracleai-memory-") as tmp:
        db = await connect(str(Path(tmp) / "memory.db"), seed=False)
        try:
            user_a = 11001
            user_b = 11002
