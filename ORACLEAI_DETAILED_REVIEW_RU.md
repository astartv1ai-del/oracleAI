...
В-третьих, документация не скрывает незакрытые внешние проверки. `docs/TASKS.md` и `ORACLEAI_FINAL_AUDIT.md` прямо отделяют local PASS от real-device, provider, payment, licensing, legal и restore evidence. Такая честность повышает доверие к baseline и облегчает дальнейшую работу команды.
...
[13]: https://github.com/astartv1ai-del/oracleAI/blob/master/docs/TASKS.md "OracleAI Tasks — local checks and open production gates"

[14]: https://github.com/astartv1ai-del/oracleAI/blob/master/app/data/schema.py "OracleAI domain tables and indexes"

[15]: https://github.com/astartv1ai-del/oracleAI/blob/master/ORACLEAI_FINAL_AUDIT.md "OracleAI Final Audit — append-only reports and verification baseline"
