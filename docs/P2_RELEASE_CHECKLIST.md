...
A P2 row may be marked **Done** only after both its local automated checks and its listed external/manual gate have evidence links. Until then, `docs/TASKS.md` and this register intentionally retain the `partial`, `manual`, `staging` or `external` qualifier.
