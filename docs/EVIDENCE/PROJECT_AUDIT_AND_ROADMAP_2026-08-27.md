...
| Disposable backup/restore drill прошёл с owner isolation, но production key custody/encryption/storage/rollback не доказаны. | `infra/backup-postgres.sh`, `infra/restore-postgres.sh`, `docs/BACKUP_RESTORE_DRILL.md`. | Local synthetic PASS. | Потеря или невозможность восстановления production данных. | 🔴 Блокер до public | Large |
...
[20]: [infra/backup-postgres.sh](../../infra/backup-postgres.sh), [infra/restore-postgres.sh](../../infra/restore-postgres.sh), [docs/BACKUP_RESTORE_DRILL.md](../BACKUP_RESTORE_DRILL.md) — current PostgreSQL backup/restore path; not production sign-off.
