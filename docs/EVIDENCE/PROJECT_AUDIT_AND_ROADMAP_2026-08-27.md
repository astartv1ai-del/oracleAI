...
| 🔴 P0 | Live LLM p95 выше целевого порога: последний bounded report — `25.088 s` против цели ≤15 секунд. | Длинный ответ ухудшает first-value experience, удержание и стоимость retries; это явно обозначенный staging blocker. | `/tmp/oracleai-llm-live-report-all.json`, `docs/LLM_EVALUATION.md`, `docs/AI_SYSTEM.md`. |
...
| 🔴 P0 | Production encrypted backup/restore, storage permissions и rollback rehearsal не подтверждены. | Локальный disposable drill не заменяет восстановление production snapshot с реальным key custody и операционной процедурой. | `infra/backup-postgres.sh`, `infra/restore-postgres.sh`, `scripts/check_p004_infrastructure.py`, `docs/EVIDENCE/ORACLEAI_CONTINUATION_REPORT_2026-08-26.md`. |
...
Аудит проведён против текущего `HEAD`, а не против старых отчётов. Документы `FULL_PRODUCT_SURFACE.md`, `RELEASE/TASKS.md`, `EVIDENCE/BASELINE_2026-08-26.md` и continuation report ценны как traceability, но часть из них ссылается на предыдущий baseline commit; поэтому все существенные выводы сверялись с текущими маршрутами, сервисами, схемами и скриптами [2] [12]. Секреты, реальные Telegram ID и содержимое пользовательских данных в документ не включались.
...
| Архитектура разделяет deterministic evidence и LLM interpretation. | `app/core/agent.py`, `app/core/agents/base.py`, `app/services/chat.py`, `docs/AI_SYSTEM.md`. | Реализовано с offline fallback и evidence contracts. | Regression при provider/model changes. | 🟡 Важно | Medium |
...
| Older audit docs refer to previous commit, while current HEAD contains subsequent changes. | `docs/FULL_PRODUCT_SURFACE.md:4`, `docs/RELEASE/TASKS.md:4`, current `git log`. | Traceability есть, но документы нельзя считать current snapshot без сверки. | Команда может принять старый статус за текущий. | 🟡 Важно | Small |
...
| Palm surface хранит structured result/hash/metadata, а raw image не возвращается как retained artifact. | `app/core/palm/`, `app/api/routers/placements.py`. | Реализовано с upload validation и owner scope. | Accuracy на consented real captures и device latency не подтверждены. | 🟡 Важно | Large |
...
| Memory consent, deletion, owner isolation и raw-image non-retention имеют server-side controls. | `app/api/routers/profile.py`, `app/services/chat.py`, `app/core/palm/`, security tests. | Сильный local baseline. | Production storage permissions, retention/legal wording и restore procedure не подтверждены. | 🔴 Блокер до public | Large |
...
| Локальный PostgreSQL backup/restore contract прошёл статические проверки, но production key custody/encryption/storage/rollback не доказаны. | `infra/backup-postgres.sh`, `infra/restore-postgres.sh`, `scripts/check_p004_infrastructure.py`. | Local synthetic PASS. | Потеря или невозможность восстановления production данных. | 🔴 Блокер до public | Large |
...
| Provider-side live LLM p95 документирован как 25.088 s против цели ≤15 s. | `/tmp/oracleai-llm-live-report-all.json`, `docs/LLM_EVALUATION.md`, `docs/AI_SYSTEM.md`. | Explicit staging blocker. | Потеря first-value, retries и рост стоимости. | 🔴 Блокер до public | Large |
...
| Находка | Фактическое доказательство | Текущее состояние | Риск | Критичность | Сложность |
|---|---|---|---|---|---|
| Четыре agent profile имеют bounded skills/tools, domain rules и safety boundaries. | `app/core/agents/`, `docs/AGENT_ARCHITECTURE.md`, `docs/AI_SYSTEM.md`. | Реализовано; context-contract check PASS. | Provider/model change может изменить поведение. | 🟡 Важно | Medium |
...
| Palm CV returns bounded summaries and never raw mask; model integrity hash is checked. | `app/core/palm_lines.py`, `app/core/palm/`, `models/THIRD_PARTY_NOTICES.md`. | Good defense-in-depth. | Model generalization, device inference and real capture benchmark open. | 🟡 Важно | Large |
...
| Deterministic Playwright baseline is documented as 6 viewports × 9 states with no horizontal overflow, unnamed focusables or missing image alt. | `docs/EVIDENCE/LOCAL_BROWSER_BASELINE_2026-08-27.md`, `scripts/capture_visual_baseline.py`. | Automated baseline claimed and current repo checks pass. | Desktop, manual keyboard/screen reader/contrast, loading/error states open. | 🟡 Важно | Medium |
...
[8]: [app/core/palm/](../../app/core/palm/), [app/core/palm_lines.py](../../app/core/palm_lines.py) — upload validation, raw-image boundary и auxiliary CV evidence.
...
[12]: [EVIDENCE/ORACLEAI_CONTINUATION_REPORT_2026-08-26.md](ORACLEAI_CONTINUATION_REPORT_2026-08-26.md) — последний локальный implementation report и explicit external blockers.
[13]: [LLM_AGENT_TECHNICAL_AUDIT.md](../AI_SYSTEM.md) — agent context, safety, quality and live latency evidence.
[14]: [DESIGN_SYSTEM.md](../DESIGN_SYSTEM.md) и [EVIDENCE/LOCAL_BROWSER_BASELINE_2026-08-27.md](LOCAL_BROWSER_BASELINE_2026-08-27.md) — design tokens, responsive/accessibility contract и browser baseline.
...
[20]: [infra/backup-postgres.sh](../../infra/backup-postgres.sh), [infra/restore-postgres.sh](../../infra/restore-postgres.sh) и [scripts/check_p004_infrastructure.py](../../scripts/check_p004_infrastructure.py) — PostgreSQL backup/restore contract and infrastructure evidence, not production sign-off.
