...
Новый implementation pass выполнен автономно по полному реестру рекомендаций из [NEXT_STEPS.md](NEXT_STEPS.md) и authoritative backlog [TASKS.md](TASKS.md). Дополнительно закрыты локальные gaps по memory evaluation, API resilience, account deletion, Tarot invariants, PDF golden cases, backup/restore drill, performance measurement, seeded browser visual coverage и end-to-end LLM-agent context integrity. Добавлены единые untrusted-data wrappers, consistency gate, intent-gated pre-tools и auxiliary palm-line CV evidence. Все изменения проходят локальные gates; public launch по-прежнему не заявляется.
