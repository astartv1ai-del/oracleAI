...
Server-side privacy guard присутствует в profile router, chat service, агентском runtime и diary router: при отключённой памяти сохранённые факты и личные дневниковые записи не должны попадать в контекст проводника.[2] Любое новое место извлечения контекста обязано применять эту же проверку.
...
| Правило | Требование |
|---|---|
| Проверка webhook | Проверять подпись Paddle до изменения заказа/entitlement. |
| Idempotency | Одно событие провайдера не должно начислить доступ повторно. |
...
- [ ] Webhook signature и idempotency проверены в sandbox.
