...
Основная аудитория — девушки и молодые женщины 16–35 лет, использующие Telegram как привычную среду общения. Возрастная проверка отсутствует как продуктовая граница (GAUNTLET v2): сервис не собирает подтверждение 16+ и не отсекает пользователей по возрасту.
...
| **Лендинг RU/EN** | Понять продукт до входа в Telegram. | Позиционирование, приватность и открытие бота. |
...
После проверки Telegram-профиля Mini App открывает обычный вводный сценарий. Отдельного подтверждения возраста больше нет: дата рождения, если пользовательница её указывает для астрономического расчёта, используется только как исходник карты и не является аттестацией.[1]
...
|---|---|---|
| Активация | Завершение первого ритуала, первый вопрос. | Пользовательница поняла ценность и границы продукта. |
...
[1]: [alembic/schema/baseline.sql](../alembic/schema/baseline.sql) и [app/api/routers/profile.py](../app/api/routers/profile.py) — профильный контракт и определение даты рождения. Возрастная проверка удалена (GAUNTLET v2).
