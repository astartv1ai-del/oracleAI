...
**Evidence:** [`LOCAL_BROWSER_BASELINE.md`](LOCAL_BROWSER_BASELINE.md), [`VISUAL_QA_A11Y_REPORT.md`](VISUAL_QA_A11Y_REPORT.md), [`UI_PIXEL_AUDIT.md`](UI_PIXEL_AUDIT.md) и воспроизводимые capture scripts [`scripts/visual_qa_capture.py`](../scripts/visual_qa_capture.py), [`scripts/capture_visual_baseline.py`](../scripts/capture_visual_baseline.py) и [`scripts/admin_visual_contract.py`](../scripts/admin_visual_contract.py). Generated JSON/PNG outputs остаются за пределами release tree и должны прикладываться к конкретному CI/run artifact.
