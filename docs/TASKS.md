...
| P2-005 | Avatar / uploads | Palm upload integration already validates MIME, malformed/undersized/oversized files, JPEG/PNG/WebP normalization, low-quality fallback and raw-image non-retention. | Define upload validation, size/type limits, retention/deletion, privacy and low-quality UI states; test malicious/oversized files. | Improved locally for palm path; avatar retention and storage lifecycle remain open |
...
| P2-008 | Growth / monetization UX | Billing and negative entitlement logic are locally tested; provider settlement and full paywall/cancellation UX are not simulated. | Verify transparent pricing, entitlement errors, trial, cancellation and refund copy with no dark patterns. | Partial: local API coverage; provider sandbox and visual UX review open |
