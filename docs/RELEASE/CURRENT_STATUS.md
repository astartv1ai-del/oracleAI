...
| **Source of truth** | Current checked-out candidate code, tests, CI configuration and [`RELEASE/TASKS.md`](TASKS.md), after synchronizing with `origin/master`; reconcile with `master` before promotion. |
| **Scope** | Repository state observed on 2026-08-27 on candidate branch `p0-004-infrastructure`; the latest merge commit and CI result must be recorded after this conflict-resolution commit. |
