...
PLANS = [
    ...
    ("vip_year", "✦✦✦ VIP на год", "восемь месяцев вместо двенадцати",
     ...
     ["Всё из VIP", "Год вместо месяца", "+300 ✦ сразу", "Соляр на год в подарок"],
     "выгода 43%", 40, 1),
    ("concierge", "👑 Консьерж", "безлимит, приоритет и эксклюзивные разборы",
     ...
     ["До 30 вопросов в день", "Приоритетная очередь", "Аудио-прогнозы",
      "Соляр и кармические разборы", "+200 ✦ каждый месяц"],
     "приоритет", 50, 1),
]
...
PRODUCTS = [
    ...
    ("report_synastry", "report", "Синастрия: полный разбор пары",
     "Две карты рядом: притяжение, трение, перспектива", 690, 90,
    ...
    ("report_solar", "report", "Соляр — прогноз на год",
     "Год от дня рождения: темы, пики, лучшие месяцы для решений", 990, 130,
