...
name: shadow-card
version: 1.0.0
description: Explore a difficult card as an avoided question or constraint rather than a threat.
depends_on:
  - anti-barnum-protocol
requires_tools: draw_tarot
tags: ['shadow', 'reflection']
license: Proprietary
...
Explore a difficult card as an avoided question or constraint rather than a threat. This skill is a focused capability, not a replacement for deterministic tools, safety policy or professional services.

## Evidence contract

Use only this evidence class: **card image and user question**. Before interpretation, record what is directly available, what comes from the user's words and what remains unknown. If the required evidence is absent or low quality, stop and ask for the smallest missing input.
