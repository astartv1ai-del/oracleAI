...
version: 1.0.0
description: Synthesize multiple aspects as a tension/resource pattern with orb and precision.
depends_on:
  - anti-barnum-protocol
requires_tools: get_chart get_all_placements
tags: ['aspects', 'synthesis']
license: Proprietary
...
Synthesize multiple aspects as a tension/resource pattern with orb and precision. This skill is a focused capability, not a replacement for deterministic tools, safety policy or professional services.

## Evidence contract

Use only this evidence class: **tool-returned aspects and orb**. Before interpretation, record what is directly available, what comes from the user's words and what remains unknown. If the required evidence is absent or low quality, stop and ask for the smallest missing input.
...
One aspect cannot define personality or cause behavior.
