...
async def list_price_items(db, *, item_type: str | None = None,
    ...
    clauses = ["price_book_version=(SELECT price_book_version FROM catalog_versions WHERE status='active' ORDER BY effective_from DESC LIMIT 1)"]
    params: list[object] = []
    if item_type:
        clauses.append("item_type=?")
        params.append(item_type)
...
async def get_subscription_state(db, tg_id: int) -> dict | None:
    cur = await db.execute("SELECT * FROM subscription_state WHERE tg_id=?", (tg_id,))
    row = await cur.fetchone()
    return dict(row) if row else None
...
async def upsert_subscription_state(db, tg_id: int, *, tier_code: str,
    ...
    async with transaction(db):
        await db.execute(
            ...
            "tg_id,tier_code,catalog_version,price_book_version,status,period_start,period_end,"
            "cancel_at_period_end,grace_until,ai_message_limit,ai_messages_used,compute_budget_usd,"
            "compute_used_usd,monthly_crystals_granted,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) "
            "ON CONFLICT(tg_id) DO UPDATE SET tier_code=excluded.tier_code, "
            "catalog_version=excluded.catalog_version, price_book_version=excluded.price_book_version, "
            ...
            "ai_message_limit=excluded.ai_message_limit, compute_budget_usd=excluded.compute_budget_usd, "
            "monthly_crystals_granted=excluded.monthly_crystals_granted, updated_at=excluded.updated_at",
            (tg_id, tier_code, catalog_version, price_book_version, status, period_start,
             period_end, int(cancel_at_period_end), grace_until, max(0, int(ai_message_limit)),
             0, max(0.0, float(compute_budget_usd or 0)), 0.0,
             max(0, int(monthly_crystals_granted or 0)), now))
...
async def reserve_usage(db, tg_id: int, operation_key: str, *, capability: str,
    ...
    async with transaction(db):
        cur = await db.execute(
            "SELECT * FROM monetization_usage WHERE tg_id=? AND operation_key=?",
            (tg_id, operation_key))
        existing = await cur.fetchone()
        if existing:
            return dict(existing)
        cur = await db.execute(
            "INSERT INTO monetization_usage(tg_id,operation_key,capability,sku,catalog_version,"
            "tier_code,period_start,units,compute_cost_usd,crystal_cost,charged_source,status,created_at,updated_at) "
            "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (tg_id, operation_key, capability, sku, catalog_version, tier_code, period_start,
             1, max(0.0, float(compute_cost_usd or 0)), max(0, int(crystal_cost or 0)),
             charged_source, "reserved", now, now))
        usage_id = cur.lastrowid
    cur = await db.execute("SELECT * FROM monetization_usage WHERE id=?", (usage_id,))
    row = await cur.fetchone()
    return dict(row) if row else None
...
async def finish_usage(db, tg_id: int, operation_key: str, *, status: str) -> bool:
    ...
    async with transaction(db):
        cur = await db.execute(
            "UPDATE monetization_usage SET status=?, updated_at=? "
            "WHERE tg_id=? AND operation_key=? AND status='reserved'",
            (status, utcnow(), tg_id, operation_key))
...
async def crystal_lot(db, tg_id: int, *, source: str, qty: int,
    ...
    async with transaction(db):
        cur = await db.execute(
            "INSERT INTO crystal_lots(tg_id,source,order_id,original_qty,remaining_qty,expires_at,created_at) "
            "VALUES(?,?,?,?,?,?,?)",
            (tg_id, source, order_id, qty, qty, expires, utcnow()))
...
async def ensure_legacy_lot(db, tg_id: int, current_balance: int) -> None:
    if current_balance <= 0:
        return
    cur = await db.execute(
        "SELECT 1 FROM crystal_lots WHERE tg_id=? AND remaining_qty>0 LIMIT 1", (tg_id,))
...
async def allocate_lots(db, tg_id: int, amount: int) -> list[tuple[int, int]]:
    ...
    async with transaction(db):
        cur = await db.execute(
            ...
            "AND (expires_at IS NULL OR expires_at>?) ORDER BY (source='purchased'), expires_at, id",
            (tg_id, now))
...
async def increment_ai_usage(db, tg_id: int, limit: int) -> bool:
    ...
    async with transaction(db):
        cur = await db.execute(
            "UPDATE subscription_state SET ai_messages_used=ai_messages_used+1, updated_at=? "
            "WHERE tg_id=? AND status IN ('active','cancelled','grace') "
            "AND (ai_message_limit=0 OR ai_messages_used < ?)",
            (utcnow(), tg_id, max(0, int(limit))))
        return bool(cur.rowcount)
...
async def set_cancel_at_period_end(db, tg_id: int, cancel: bool = True) -> dict | None:
    async with transaction(db):
        cur = await db.execute(
            "UPDATE subscription_state SET cancel_at_period_end=?, status=CASE "
            "WHEN ?=1 AND status='active' THEN 'cancelled' WHEN ?=0 AND status='cancelled' THEN 'active' ELSE status END, updated_at=? "
            "WHERE tg_id=?",
            (int(cancel), int(cancel), int(cancel), utcnow(), tg_id))
        if not cur.rowcount:
            return None
...
async def transition_expired(db, tg_id: int) -> dict | None:
    async with transaction(db):
        cur = await db.execute(
            "UPDATE subscription_state SET status='expired', tier_code='free', ai_message_limit=0, "
            "compute_budget_usd=0, updated_at=? WHERE tg_id=? AND period_end IS NOT NULL "
            "AND period_end<=? AND status IN ('active','cancelled','grace')",
            (utcnow(), tg_id, utcnow()))
        if not cur.rowcount:
            return await get_subscription_state(db, tg_id)
    return await get_subscription_state(db, tg_id)
...
async def dashboard(db, days: int = 30) -> dict:
    ...
    cur = await db.execute(
        ...
        "WHERE status IN ('active','cancelled','grace') GROUP BY tier_code ORDER BY users DESC")
    active_by_tier = {row["tier_code"]: int(row["users"] or 0) for row in await cur.fetchall()}
    cur = await db.execute(
        "SELECT COALESCE(sku,'unknown') sku, COUNT(*) orders, COALESCE(SUM(amount_stars),0) stars "
        "FROM orders WHERE status='paid' AND paid_at>=? GROUP BY sku ORDER BY stars DESC", (since,))
    revenue_by_sku = [{"sku": row["sku"], "orders": int(row["orders"] or 0), "stars": int(row["stars"] or 0)} for row in await cur.fetchall()]
    cur = await db.execute(
        "SELECT COALESCE(reason,'unknown') source, COALESCE(SUM(CASE WHEN delta>0 THEN delta ELSE 0 END),0) granted, "
        "COALESCE(SUM(CASE WHEN delta<0 THEN -delta ELSE 0 END),0) spent "
        "FROM crystal_ledger WHERE created_at>=? GROUP BY reason", (since,))
    crystal_by_source = [{"source": row["source"], "granted": int(row["granted"] or 0), "spent": int(row["spent"] or 0)} for row in await cur.fetchall()]
    cur = await db.execute(
        "SELECT event_kind, COUNT(*) events, COALESCE(SUM(cost_usd),0) cost_usd "
        "FROM product_cost_events WHERE created_at>=? GROUP BY event_kind", (since,))
    cogs_by_kind = [{"event_kind": row["event_kind"], "events": int(row["events"] or 0), "cost_usd": round(float(row["cost_usd"] or 0), 6)} for row in await cur.fetchall()]

    names = (
        "paywall_view", "paywall_choice", "invoice_created", "paid", "first_paid_action",
        "credit_pack_checkout_started", "credit_pack_paid", "credit_spent",
        "subscription_lifecycle", "report_delivered", "refund_completed",
    )
    funnel = {}
    for name in names:
        funnel[name] = int(await scalar("SELECT COUNT(*) FROM events WHERE name=? AND created_at>=?", (name, since)))

    paid_orders = int(await scalar("SELECT COUNT(*) FROM orders WHERE status='paid' AND paid_at>=?", (since,)))
    refunds = int(await scalar("SELECT COUNT(*) FROM orders WHERE status='refunded' AND created_at>=?", (since,)))
    repeat = int(await scalar(
        "SELECT COUNT(*) FROM (SELECT tg_id FROM orders WHERE status='paid' AND paid_at>=? GROUP BY tg_id HAVING COUNT(*)>1)",
        (since,)))
    outstanding = int(await scalar("SELECT COALESCE(SUM(remaining_qty),0) FROM crystal_lots WHERE remaining_qty>0"))
    purchased = int(await scalar("SELECT COALESCE(SUM(remaining_qty),0) FROM crystal_lots WHERE source='purchased' AND remaining_qty>0"))
    bonus = int(await scalar("SELECT COALESCE(SUM(remaining_qty),0) FROM crystal_lots WHERE source='subscription_bonus' AND remaining_qty>0"))
    return {
        "window_days": int(days), "since": since,
        "revenue": {"paid_orders": paid_orders, "refunded_orders": refunds,
                    "refund_rate_pct": round(refunds * 100 / paid_orders, 2) if paid_orders else 0.0,
                    "gross_stars": int(await scalar("SELECT COALESCE(SUM(amount_stars),0) FROM payments WHERE status='succeeded' AND created_at>=?", (since,))),
                    "by_sku": revenue_by_sku, "net_revenue_estimate": None},
        "subscription": {"active_by_tier": active_by_tier,
                         "new_paid_users": int(await scalar("SELECT COUNT(DISTINCT tg_id) FROM orders WHERE status='paid' AND paid_at>=?", (since,))),
                         "repeat_payers": repeat, "renewal_and_churn": "not_available_without provider lifecycle exports"},
        "crystals": {"by_source": crystal_by_source, "outstanding": outstanding,
                     "purchased_outstanding": purchased, "bonus_outstanding": bonus,
                     "repeat_purchase_users": int(await scalar("SELECT COUNT(*) FROM (SELECT tg_id FROM orders WHERE kind='crystals' AND status='paid' AND paid_at>=? GROUP BY tg_id HAVING COUNT(*)>1)", (since,)))},
        "funnel": funnel, "cogs": {"by_kind": cogs_by_kind,
            "total_variable_cost_usd": round(sum(item["cost_usd"] for item in cogs_by_kind), 6)},
        "contribution_margin_estimate": None,
        "required_inputs": ["provider_settlement_realization", "tax_and_withholding_rate", "refund_rate_by_channel", "fixed_opex", "paid_marketing", "provider_lifecycle_export"],
    }
...
async def assign_variant(db, tg_id: int, experiment: str,
    ...
    cur = await db.execute(
        "SELECT variant FROM monetization_assignments WHERE tg_id=? AND experiment=?",
        (tg_id, safe_experiment))
    row = await cur.fetchone()
    if row:
        return row["variant"]
    ...
    async with transaction(db):
        await db.execute(
            "INSERT OR IGNORE INTO monetization_assignments(tg_id,experiment,variant,assigned_at) VALUES(?,?,?,?)",
            (tg_id, safe_experiment, variant, utcnow()))
    cur = await db.execute(
        "SELECT variant FROM monetization_assignments WHERE tg_id=? AND experiment=?",
        (tg_id, safe_experiment))
    row = await cur.fetchone()
